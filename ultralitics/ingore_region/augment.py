...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            labels_patch = labels if i == 0 else labels["mix_labels"][i - 1]
            imap = labels_patch.pop("ignore_map", None)
            ...
            if imap is not None:
                maps_and_offsets.append((imap, padw, padh))

            mosaic_labels.append(labels_patch)
        final_labels = self._cat_labels(mosaic_labels)
        final_labels["img"] = img4

        # build the full-size ignore_map
        Hm, Wm = img4.shape[:2]
        ignore4 = np.zeros((Hm, Wm), dtype=np.uint8)
        for imap, padw, padh in maps_and_offsets:
            h_i, w_i = imap.shape
            y1, y2 = padh, padh + h_i
            x1, x2 = padw, padw + w_i
            # clip in case a patch ran off the edge
            y1c, y2c = max(y1, 0), min(y2, Hm)
            x1c, x2c = max(x1, 0), min(x2, Wm)
            sy, sx = y1c - y1, x1c - x1  # how much we had to shift the source
            if y2c > y1c and x2c > x1c:
                ignore4[y1c:y2c, x1c:x2c] = imap[
                    sy : sy + (y2c - y1c), sx : sx + (x2c - x1c)
                ]

        final_labels["ignore_map"] = ignore4
    ...
    def _update_labels(labels, padw, padh):
        ...
        if "ignore_map" in labels:
            imap = labels["ignore_map"]  # old tile’s H×W mask
            H, W = imap.shape
            # create full-canvas mask
            padded = np.zeros((nh, nw), dtype=imap.dtype)
            ...
            # source region within imap
            sy1 = max(-padh, 0)
            sx1 = max(-padw, 0)
            sy2 = sy1 + (y2 - y1)
            sx2 = sx1 + (x2 - x1)

            if y2 > y1 and x2 > x1:  # only if there’s overlap
                padded[y1:y2, x1:x2] = imap[sy1:sy2, sx1:sx2]

            labels["ignore_map"] = padded
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        if "ignore_map" in labels and "ignore_map" in labels2:
            im1 = labels["ignore_map"]
            im2 = labels2["ignore_map"]
            # elementwise AND will zero out any pixel that was ignored in either image
            labels["ignore_map"] = (im1.astype(bool) & im2.astype(bool)).astype(
                im1.dtype
            )
            # labels["ignore_map"] = (im1 & im2).astype(np.uint8)
...
class CutMix(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        # ─── now propagate the ignore_map through the same patch ───
        # if both inputs provided an ignore_map, splice them here:
        if "ignore_map" in labels and "ignore_map" in labels2:
            imap1 = labels["ignore_map"]
            imap2 = labels2["ignore_map"]
            new_imap = imap1.copy()
            new_imap[y1:y2, x1:x2] = imap2[y1:y2, x1:x2]
            labels["ignore_map"] = new_imap
        else:
            # if missing, assume “no ignore” everywhere
            H, W = labels["img"].shape[:2]
            Hm, Wm = H // self.mask_ratio, W // self.mask_ratio
            labels["ignore_map"] = np.ones((Hm, Wm), dtype=np.uint8)
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        if "ignore_map" in labels:
            imap = labels.pop("ignore_map")  # H×W uint8
            h, w = img.shape[:2]
            imap_warped = cv2.warpPerspective(
                imap,
                M,
                (w, h),
                flags=cv2.INTER_NEAREST,
                borderMode=cv2.BORDER_CONSTANT,
                borderValue=0,  # pixels outside get “ignore”=0
            )
            labels["ignore_map"] = imap_warped
        return labels
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        """
        ignore_map = labels.get("ignore_map", None)
        ...
        # 2) restore ignore_map into labels unchanged
        if ignore_map is not None:
            labels["ignore_map"] = ignore_map
...
class RandomFlip:
    ...
    def __call__(self, labels):
        ...
        ignore_map = labels.pop("ignore_map", None)
        ...
        if ignore_map is not None:
            # ensure same dtype as downstream expects (e.g. uint8 or float32)
            labels["ignore_map"] = ignore_map
        return labels
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if len(labels):
            # 1) resize+pad ALL of the normal annotations (bboxes, segments, keypoints…)
            labels = self._update_labels(labels, ratio, left, top)
            labels["img"] = img
            labels["resized_shape"] = new_shape

            if "ignore_map" in labels:
                imap = labels.pop("ignore_map")  # Original HxW map

                # 1. Apply the same resize operation as the image
                if tuple(imap.shape) != tuple(shape): # If imap shape is not the same as original image shape
                    imap = cv2.resize(imap, (shape[1], shape[0]), interpolation=cv2.INTER_NEAREST)
                if shape[::-1] != new_unpad:
                    imap = cv2.resize(imap, new_unpad, interpolation=cv2.INTER_NEAREST)

                # 2. Get the actual dimensions of the resized imap
                h_imap, w_imap = imap.shape[:2]

                # 3. Create a padded canvas and place the resized imap into it
                #    The slice is defined by h_imap and w_imap to guarantee a shape match.
                padded_imap = np.full((h_imap + top + bottom, w_imap + left + right), 0, dtype=imap.dtype)
                padded_imap[top:top + h_imap, left:left + w_imap] = imap
                labels["ignore_map"] = padded_imap
...
class Format:
    ...
    def __call__(self, labels):
        ...
        # print(f"[DEBUG][Format._format_segments] masks.shape: {masks.shape}")
        ...
        # A: carry through ignore_map if the label loader put one there
        if "ignore_map" in labels:
            # our raw ignore_map is full-resolution numpy H×W, values {0,1}
            imap = labels.pop("ignore_map")  # (H, W) np.uint8
            # Hm = h // self.mask_ratio
            # Wm = w // self.mask_ratio

            # # nearest-neighbor downsample to match mask size
            # imap_rs = cv2.resize(imap, (Wm, Hm), interpolation=cv2.INTER_NEAREST)

            # # convert to tensor [Hm, Wm]
            # labels["ignore_map"] = torch.from_numpy(imap_rs).float()
            labels["ignore_map"] = torch.from_numpy(imap).float()
        # print("[Format] final keys:", list(labels.keys()))
        return labels
