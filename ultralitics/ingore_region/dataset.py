...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self):
        ...
        if not labels:
            LOGGER.warning(
                f"No images found in {cache_path}, training may not work correctly. {HELP_URL}"
            )
        ...
        if len_cls == 0:
            LOGGER.warning(
                f"No labels found in {cache_path}, training may not work correctly. {HELP_URL}"
            )
        return labels
    ...
    def update_labels_info(self, label):
        """
        Custom your label format here, plus build an ignore_map
        for all class‐0 (low‐certainty) polygons.
        """
        # In __getitem__ after label is loaded/constructed:
        # 1) pull out the things you were already doing
        ...
        # 2) your existing segment‐resample logic
        segment_resamples = 1000 if not self.use_obb else 100
        if len(segments) > 0:
            max_len = max(len(s) for s in segments)
            n = max(segment_resamples, max_len + 1)
            segments = np.stack(resample_segments(segments, n=n), axis=0)
        else:
            segments = np.zeros((0, segment_resamples, 2), dtype=np.float32)

        # 3) build the Instances object as before
        ...
        # 4) now build ignore_map
        #    assume label["ori_shape"] == (H, W)
        H, W = label["ori_shape"]
        # 1 = keep, 0 = ignore
        ignore_map = np.ones((H, W), dtype=np.uint8)
        # ignore_map = np.zeros((H, W), dtype=np.uint8)

        # get your class array (shape (N,1) or (N,))
        cls = label["cls"].reshape(-1)

        for seg, c in zip(segments, cls):
            if int(c) == 0:  # low-certainty
                # seg: either normalized [0..1] or absolute coords
                if normalized:
                    pts = np.round(seg * [W, H]).astype(np.int32)
                else:
                    pts = seg.astype(np.int32)
                # fill it
                cv2.fillPoly(ignore_map, [pts], 0)

        # print(
        #     f"DEBUG [Dataset] ignore_map min={ignore_map.min()}, max={ignore_map.max()}, sum={ignore_map.sum()} / {ignore_map.size}"
        # )

        label["ignore_map"] = (
            ignore_map  # now downstream you’ll get batch["ignore_map"]
        )
        # print(f"[v2 update_labels_info] keys before return: {list(label.keys())}")
    ...
    def collate_fn(batch):
        """
        Collates data samples into batches, now including `ignore_map`.
        """
        new_batch = {}
        ...
        for i, k in enumerate(keys):
            v = values[i]
            # images, text_feats and our new ignore_map get stacked
            if k in {"img", "text_feats", "ignore_map"}:
                new_batch[k] = torch.stack(v, 0)
            # visuals get padded
            elif k == "visuals":
                new_batch[k] = torch.nn.utils.rnn.pad_sequence(v, batch_first=True)
            else:
                # masks / bboxes / cls / etc get concatenated
                if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb"}:
                    new_batch[k] = torch.cat(v, 0)
                else:
                    new_batch[k] = v  # other metadata

        # re-build batch_idx exactly as before
        new_batch["batch_idx"] = list(new_batch["batch_idx"])
        for i in range(len(new_batch["batch_idx"])):
            new_batch["batch_idx"][i] += i
        new_batch["batch_idx"] = torch.cat(new_batch["batch_idx"], 0)
        # print("[collate_fn] new_batch keys:", list(new_batch.keys()))
        # print(
        #     f"[DEBUG][collate_fn] batch['masks'].shape: {new_batch['masks'].shape}, batch['cls'].shape: {new_batch['cls'].shape}, batch_idx: {new_batch['batch_idx'].shape}"
        # )
...
class YOLOMultiModalDataset(YOLODataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(
        #     f"[DEBUG][Dataset __getitem__] img idx: {idx}, cls: {label['cls'].shape}, segments: {np.array(label['segments']).shape if 'segments' in label else None}"
        # )
        return label
    ...
    def update_labels_info(self, label):
        ...
        # print(
        #     f"[DEBUG][update_labels_info] instances: {label['instances'] if 'instances' in label else None}"
        # )
