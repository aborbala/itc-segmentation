...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # # Save only the first few images to avoid flooding your drive
        # if index < 5:
        #     # Revert image from CHW tensor to HWC numpy array for saving
        #     img_tensor = label['img']
        #     img_np = img_tensor.permute(1, 2, 0).numpy().astype(np.uint8)
            
        #     # Revert ignore_map from tensor to numpy array for saving
        #     # It should be float, so scale by 255 to make it a visible image
        #     ignore_map_tensor = label['ignore_map']
        #     ignore_map_np = (ignore_map_tensor.numpy() * 255).astype(np.uint8)

        #     # Save the files
        #     cv2.imwrite(f'debug_img_{index}.png', img_np)
        #     cv2.imwrite(f'debug_ignore_map_{index}.png', ignore_map_np)
        #     print(f"Saved debug images for index {index}")
        # # END: DEBUGGING BLOCK
    ...
    def update_labels_info(self, label):
        ...
        segments = label["segments"]  # list of [x1,y1,x2,y2,…] polygons
        classes = label["cls"].flatten()  # e.g. array([0,1,1,0,1])
        h_resized, w_resized = label["resized_shape"]
        h_orig, w_orig = label["ori_shape"]

        # create a full-ones mask
        ignore_map = np.ones((h_resized, w_resized), dtype=np.uint8)

        # scale factor from original coords → resized coords
        sy = h_resized / h_orig
        sx = w_resized / w_orig

        # for every polygon whose class == 0, rasterize it to 0 in ignore_map
        polys_to_ignore = []
        for seg, c in zip(segments, classes):
            if int(c) == 0:
                pts = np.array(seg, dtype=np.float32).reshape(-1, 2)
                pts[:, 0] *= sx  # x coord
                pts[:, 1] *= sy  # y coord
                polys_to_ignore.append(pts.astype(np.int32))

        if polys_to_ignore:
            cv2.fillPoly(ignore_map, polys_to_ignore, 0)

        # finally, stash it back on the label dict as a torch tensor
        label["ignore_map"] = torch.from_numpy(ignore_map)  # will be (H,W)
