...
try:
    ...
    if not output_files:
        print("  ℹ️ Output directory is empty. Processing all rasters.")
        ...
        for f in output_files:
            ...
            try:
                ...
                print(f"  ⚠️ Warning: Could not parse output filename {fname}")
        ...
        # 3. Report findings
        print(f"  ✅ Found {fully_completed_count} fully completed rasters (>= {EXPECTED_WINDOWS_PER_RASTER} windows).")
        if not rasters_to_process:
            print("  🎉 All rasters are already fully processed! Nothing to do.")
        else:
            print(f"  ▶️ {len(rasters_to_process)} rasters remain to be processed.")
            print(f"  ℹ️ Starting with: {os.path.basename(rasters_to_process[0])}")

except Exception as e:
    print(f"  ❌ Error checking for resume status: {e}. Processing all {len(all_image_paths)} rasters.")
    rasters_to_process = all_image_paths
...
for i, raster_path in enumerate(rasters_to_process):
    ...
    try:
        with rasterio.open(raster_path) as src:
            ...
            if crs != TARGET_CRS:
                print(f"   ⚠️ WARNING: Source raster CRS is {crs}, but target is {TARGET_CRS}.")
                print(f"     All outputs will be in the raster's CRS: {crs}")
                ACTIVE_CRS = crs
            else:
                print(f"   ✅ Raster CRS confirmed: {crs}")
            ...
            print(f"   ℹ️ Raster will be processed in {total_windows} windows (Tile={tile_size_x}px, Step={step_y}px).")
            
            # --- Sanity check ---
            if total_windows != EXPECTED_WINDOWS_PER_RASTER:
                print(f"  ⚠️⚠️⚠️ WARNING: Your EXPECTED_WINDOWS_PER_RASTER is {EXPECTED_WINDOWS_PER_RASTER},")
                print(f"  ⚠️⚠️⚠️ but this raster calculated {total_windows} windows. The resume logic may fail *next* time if this is wrong.")
                ...
                for col_start in col_positions:
                    ...
                    try:
                        ...
                        if results[0].masks is None or len(results[0].masks.data) == 0:
                            ...
                            if window_count % 50 == 0 or window_count == 1:
                                print(f"       ✅ Predictions: {len(gdf_pred)}")
                        ...
                        print(f"     ❌ ERROR processing window {window_aoi_code}: {e}")
                        all_preds_gdfs.append(gpd.GeoDataFrame(geometry=[], crs=ACTIVE_CRS))

    except Exception as e:
        print(f"   ❌ ERROR opening or processing large raster {raster_path}: {e}")
...
for i, raster_path in enumerate(image_paths):
    ...
    try:
        ...
        if os.path.exists(gt_path):
            ...
            print(f"  ✅ Loaded {len(gdf_gt)} ground truth polygons.")
        else:
            print(f"  ⚠️ Digitized ground truth not found for {aoi_code}, skipping GT.")
        ...
        if results[0].masks is None or len(results[0].masks.data) == 0:
            ...
            print(f"  ✅ Found {len(gdf_pred)} initial predictions.")
        ...
        print(f"  ❌ ERROR processing {aoi_code}: {e}")
