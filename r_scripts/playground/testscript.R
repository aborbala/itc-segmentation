...
#dop_tile     <- "386_5818"
#sub_dop_tile <- "386_5819"  
#img_nr       <- 98  

dop_tile     <- "384_5816"
sub_dop_tile <- "384_5817"  
img_nr       <- 59
...
# info
# - 3dm_33_386_5818_1_be_nobuild_40.tif <- mostly only cadaster trees: urban
# - 3dm_33_387_5819_1_be_nobuild_61.tif <- mostly only cadaster trees: park
# - 3dm_33_387_5819_1_be_nobuild_83.tif <- mostly no cadaster trees: park
# - 3dm_33_386_5819_1_be_nobuild_136.tif <- mostly no cadaster trees: urban


#las_nobuild_path <- "C:/tree-canopy/data/386_5818/LAS_no_buildings/3dm_33_386_5819_1_be_nobuild.las"
#las_nobuild_path <- "H:/My Drive/masterthesis/data/386_5818/LAS_no_structures_veg_mask/3dm_33_386_5818_1_be_nobuild.las"
#tif_directory <- sprintf("H:/My Drive/masterthesis/data/%s/sliced_imgs_2020S", dop_tile)
#test <- sprintf("H:/My Drive/masterthesis/data/%s/inner_tiles/3dm_33_384_58168_1_be_nobuild_19.tif", dop_tile)
#tree_cadaster_path <- "H:/My Drive/masterthesis/data/386_5818/trees.gpkg"
#tree_cadaster_aoi <- st_read(tree_cadaster_path)

#base_path <- file.path("H:/My Drive/masterthesis/data", dop_tile)
base_path <- file.path("G:/Meine Ablage/masterthesis/data", dop_tile)

las_nobuild_path <- file.path(
  base_path,
  "LAS_no_structures_veg_mask",
  sprintf("3dm_33_%s_1_be_nobuild.las", sub_dop_tile)
)

# – tif_directory never included an img number
tif_directory <- file.path(base_path, "sliced_imgs_2020S")

#tif_dir<-"inner_tiles"
tif_dir<-"sliced_imgs_2020S"


# – test originally had both sub-tile & img number
test <- file.path(
  base_path,
  tif_dir,
  sprintf("3dm_33_%s_1_be_nobuild_%s.tif", sub_dop_tile, img_nr)
)

# – tree_cadaster_path & its read stay the same
tree_cadaster_path <- file.path(base_path, "trees.gpkg")
tree_cadaster_aoi  <- st_read(tree_cadaster_path)
st_crs(tree_cadaster_aoi) <- 25833
...
# Clip tree to Raster Extent
tree_cadaster <- st_intersection(tree_cadaster_aoi, st_as_sfc(ext))
...
# Standard plotting
layers <- c(chm_p2r_05, chm_dsmtin_base, extend(chm_dsmtin_5, ext(chm_p2r_05)), chm_pitfree_subcirlce)
...
create_chm_plot <- function(df, title) {
  ggplot(df, aes(x = x, y = y, fill = Z)) +
    geom_tile() +
    scale_fill_viridis_c(option = "mako") +
    # Remove x and y labels here
    labs(title = title, fill = "Height (m)", x = NULL, y = NULL) +
    coord_equal() +
    theme(
...
# Arrange plots in patchwork
chm_plot2 <- wrap_plots(p1, p6, ncol = 2) + 
  plot_layout(guides = 'collect') &  # Aligns legends and reduces spacing
  theme(plot.margin = margin(0, 0, 0, 0))  # Removes large gaps
...
# -----------------------------------------------
# Fitted model:
# converts from diameter to radius using / 2
ws_2nd_polynomial <- function(H) {
  return(pmax((6.2299 + 1.1495 * H - 0.0105 * H^2) / 2, 1))
}
...
ws_dec <- ws_deciduous(heights)
ws_poly <- ws_2nd_polynomial(heights)

plot(heights, ws_dec, type = "l", col = "blue", ylim = c(0, 25), 
     xlab = "Height (H)", ylab = "Window Size (ws)", 
     main = "Comparison of Fitted Model and Deciduous Tree Local Max Filter")
lines(heights, ws_poly, col = "red", lty = 2)  # Add second function
...
ttops_LAS_ws_polynomial <- locate_trees(las_unfiltered, lmf(ws_2nd_polynomial, hmin=5,  ws_args = list("Z")))
plot(sf::st_geometry(ttops_LAS_ws_polynomial), add = TRUE, pch = 3, col=color_magenta, cex = 2, lwd = 3) #blue
...
plot(sf::st_geometry(ttops_pitfree_subcirlce_deciduous), add = TRUE, pch = 2, col=color_green, cex = 2, lwd = 3) #green
ttops_pitfree_subcirlce_polynomial <- locate_trees(chm_pitfree_subcirlce, lmf(ws = ws_2nd_polynomial, hmin = 5,  ws_args = list("Z")))
plot(sf::st_geometry(ttops_pitfree_subcirlce_polynomial), add = TRUE, pch = 3, col=color_magenta, cex = 2, lwd = 3) #green
...
ttops_pitfree_subcirlce_polynomial_df <- convert_sf_to_df(ttops_pitfree_subcirlce_polynomial)
...
plot_chm_3 <- base_map +
  geom_point(data = ttops_pitfree_subcirlce_polynomial_df, aes(x = X, y = Y), color = color_yellow, size = 1, shape = 3,  stroke = 2) +
  labs(title = "Cadaster vs CHM Trees, ws = polynomial")
...
# Version 2
test_r <- rast(test)
test_df <- as.data.frame(test_r, xy = TRUE)

# Assuming the first 3 bands correspond to R, G, B columns (3, 4, 5)
# We create a 'hex_color' column using the rgb() function
# Note: Ensure data is 8-bit (0-255). If 16-bit, might need adjustment.
test_df$hex_color <- rgb(test_df[,3], test_df[,4], test_df[,5], maxColorValue = 255)

# Base ggplot layer (RGB Background + Cadaster Trees) in UTM EPSG:25833
base_map <- ggplot() +
  geom_tile(data = test_df, aes(x = x, y = y, fill = hex_color)) +
  scale_fill_identity() +
  geom_point(data = tree_cadaster_df, aes(x = X, y = Y),
             color = color_orange, size = 4, shape = 18) +
  coord_equal() +
  theme(
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
...
plot_lidar <- base_map +
  geom_point(data = ttops_LAS_df, aes(x = X, y = Y), color = color_yellow, size = 1, shape = 3,  stroke = 2) +
  labs(title = "LiDAR-based Detection")

# Compare cadaster vs CHM detected trees (pitfree), ws_combined
plot_chm_3 <- base_map +
  geom_point(data = ttops_pitfree_subcirlce_polynomial_df, aes(x = X, y = Y), color = color_yellow, size = 1, shape = 3,  stroke = 2) +
  labs(title = "CHM Detection (Polynomial)")

# Compare cadaster vs CHM detected trees (pitfree), ws_decidous
plot_chm_5 <- base_map +
  # Update aes mapping to use "Detected Treetops"
  geom_point(data = tree_cadaster_df, aes(x = X, y = Y, color = "Cadaster Tree", shape = "Cadaster Tree"), size = 4) +
  geom_point(data = ttops_pitfree_subcirlce_df, aes(x = X, y = Y, color = "Detected Treetops", shape = "Detected Treetops"), size = 1, stroke = 2) +
  
  # Update manual scales with new label name
  scale_color_manual(values = c("Cadaster Tree" = color_orange, "Detected Treetops" = color_yellow), name = "Tree Type") +
  scale_shape_manual(values = c("Cadaster Tree" = 18, "Detected Treetops" = 3), name = "Tree Type") +
  
  labs(title = "CHM Detection (Deciduous)")
...
treetop_plot <- (plot_lidar | plot_chm_3 | plot_chm_5) +
  ...
  theme(
    ...
    panel.spacing = unit(0, "pt")   # <-- removes spacing between panels
  )
...
ttops_pitfree_subcirlce_cleaned <- locate_trees(chm_pitfree_subcirlce_cleaned, lmf(ws_2nd_polynomial, hmin=5, shape = c("square")))
...
raster_raw_df <- as.data.frame(rast(test), xy = TRUE, na.rm = TRUE)

# Rename raster bands
names(raster_raw_df)[3:5] <- c("red", "green", "blue")

# Ensure all bands are numeric and not factors
raster_raw_df <- raster_raw_df %>%
  mutate(across(c(red, green, blue), as.numeric))
...
#  mutate(buffer_size = ws_2nd_polynomial(baumhoehe))
...
tree_cadaster <- tree_cadaster %>%
  mutate(
    # if kronedurch is NA or zero, use the polynomial on baumhoehe;
    # otherwise use kronedurch / 2 (radius)
    buffer_size = if_else(
      is.na(kronedurch) | kronedurch == 0,
      ws_2nd_polynomial(baumhoehe),
      kronedurch / 2
    )
  )

# now build the actual sf buffer
tree_cadaster_buffer <- st_buffer(tree_cadaster, dist = tree_cadaster$buffer_size)
...
results <- bind_rows(
  ...
  calculate_metrics(ttops_pitfree_subcirlce_polynomial, "CHM pitfree ws=polynomial"),
...
#ttops_cadaster <- tree_cadaster
#ttops <- ttops_cadaster[, c("baumhoehe", "geom")]

# Rename the columns
#colnames(ttops) <- c("Z", "geom")
# Add the treeID column as a sequence from 1 to number of rows
...
algo_dalponte <- dalponte2016(chm, ttops,   th_tree = 2,    # Minimum tree height
                              ...
                              max_cr = 35    # Max crown diameter (20 pixels = 10m for 0.5m CHM)
)
...
plot(crowns_dalponte["i_mean"], main = "z_max")
...
## Post processing: Filter out small convex hulls, consider crowns over 1m2
plot(crowns_dalponte_convex["i_ratio"], main = "z_max")
#crowns <- calculate_ratio_df(crowns)
#crowns <- crowns[crowns$width_to_height_ratio > 0.5 & crowns$width_to_height_ratio < 1.5 ,]
plot(chm, col = mako(50))
#plot(sf::st_geometry(ttops), add = TRUE, pch = 3)
plot(crowns_dalponte_convex["z_max"], col = "transparent", border = color_cyan, lwd = 3, add = TRUE)
plot(sf::st_geometry(ttops), add = TRUE, pch = 3, col=color_red, cex = 2, lwd = 3)
...
plot_dalponte <- base_map +
  geom_polygon(data = crowns_dalponte_df, aes(x = x, y = y, group = polygon_id),
               color = color_magenta, fill = NA, linewidth = 2) +
...
plot_silva <- base_map +
  ...
  geom_point(data = ttops_LAS_df, aes(x = X, y = Y, color = "Detected Treetop", shape = "Detected Treetop"),shape = 3, size = 1, stroke = 2) +
  scale_color_manual(name = "Tree Type", values = c("Cadaster Tree" = color_orange, "Detected Treetop" = color_yellow)) +
  scale_shape_manual(name = "Tree Type", values = c("Cadaster Tree" = 18, "Detected Treetop" = 3)) + 
  theme(legend.position = "right")+
...
### RandCrowns ###
compute_randcrowns <- function(pred, target, alpha = 0.7, omega = 1.2, gamma = 3.0) {
  # True Positive Region (Ra)
  ra <- st_buffer(target, -alpha)
  
  # True Negative Inner Boundary (Ro)
  ro <- st_buffer(target, omega)
  
  # Area of Ra
  area_ra <- st_area(ra)
  area_rb <- gamma * area_ra
  total_rb_area <- area_rb + st_area(ro)
  
  # Estimate buffer increment for Rb
  approx_radius_rb <- sqrt(as.numeric(total_rb_area) / pi)
  current_radius_ro <- (st_bbox(ro)[["xmax"]] - st_bbox(ro)[["xmin"]]) / 2
  tau <- approx_radius_rb - current_radius_ro
  rb <- st_buffer(target, omega + tau)
  
  # Intersections
  a <- st_area(st_intersection(pred, ra))
  b <- st_area(st_difference(rb, pred))
  c <- st_area(st_intersection(pred, rb))
  d <- st_area(st_difference(ra, pred))
  
  denom <- a + b + c + d
  if (as.numeric(denom) > 0) {
    return(as.numeric((a + b) / as.numeric(denom)))
  } else {
    return(0)
  }
}
...
# Evaluate
scores_dalponte <- evaluate_randcrowns(crowns_dalponte, tree_cadaster_buffer)
scores_silva <- evaluate_randcrowns(crowns_silva, tree_cadaster_buffer)
scores_dalponte_convex <- evaluate_randcrowns(crowns_dalponte_convex, tree_cadaster_buffer)

# Print results
cat("📊 RandCrowns Evaluation:\n")
cat(sprintf("Dalponte — Mean: %.3f, SD: %.3f, N: %d\n",
            mean(scores_dalponte), sd(scores_dalponte), length(scores_dalponte)))
cat(sprintf("Silva — Mean: %.3f, SD: %.3f, N: %d\n",
            mean(scores_silva), sd(scores_silva), length(scores_silva)))
cat(sprintf("Dalponte_convex — Mean: %.3f, SD: %.3f, N: %d\n",
            mean(scores_dalponte_convex), sd(scores_dalponte_convex), length(scores_dalponte_convex)))
# PLOTTING
# Combine into one data frame
results_df <- tibble::tibble(
  score = c(scores_dalponte, scores_silva),
  method = c(rep("Dalponte", length(scores_dalponte)),
             rep("Silva", length(scores_silva)))
)

ggplot(results_df, aes(x = method, y = score, fill = method)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.4, size = 2) +
  labs(
    title = "RandCrowns Evaluation by Method",
    x = "Crown Extraction Method",
    y = "RandCrowns Score (0–1)"
  ) +
...
results_df <- tibble::tibble(
  score = c(scores_dalponte, scores_silva),
  method = rep(c("Dalponte", "Silva"), each = length(scores_dalponte))
) %>%
  mutate(matched = ifelse(score > 0, "Matched", "Unmatched"))

summary_plot_data <- results_df %>%
  group_by(method, matched) %>%
  summarise(n = n(), .groups = "drop")

# Plot
ggplot(summary_plot_data, aes(x = method, y = n, fill = matched)) +
  geom_bar(stat = "identity", position = "dodge", color = "black", width = 0.6) +
  scale_fill_manual(
    values = c("Matched" = "#4CAF50", "Unmatched" = "#F44336"),
    name = "Match Status"
  ) +
  labs(
    title = "Matched vs. Unmatched Crowns by Method",
    x = "Method",
    y = "Number of Crowns"
  ) +
...
results_df %>%
  ...
  labs(
    title = "RandCrowns Score Distribution for Matched Trees",
    x = "Method",
    y = "RandCrowns Score"
  ) +
  theme_minimal() +
...
summary_table %>%
  kable("html", caption = "Matched vs. Unmatched Tree Crowns by Method") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover", "condensed")) %>%
  row_spec(0, bold = TRUE, color = "white", background = "#606060") %>%
  column_spec(2, background = "#c6f5c6") %>%  # Matched column
  column_spec(3, background = "#f5c6c6")      # Unmatched column
